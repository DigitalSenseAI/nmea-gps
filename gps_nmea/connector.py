...
class GPS_connector:
    ...
    def __init__(
        ...
        Args:
            ...
            strategy (str): Strategy for reading data from the gps. 'block' will read the gps output until
            it finds one with the expected message type,
            while 'yield' will return the output of first line read,
            being an empty dict when the message is not message_type.
        Return:
            None
        """
        message_function_mapping = {"GGA": self.get_gga_data, "RMC": self.get_rmc_data}
        integrity_function_mapping = {
            "GGA": self.check_gga_integrity,
            "RMC": self.check_rmc_integrity,
        }
    ...
    def get_data(self) -> dict:
        ...
        if self.strategy == "block":
            ...
            raise ValueError(
                "Invalid strategy for reading data from gps. Valid options are 'block' and 'yield'"
            )
    ...
    def check_gga_integrity(line_data: list[str]) -> bool:
        ...
        try:
            ...
            if (
                not line_data[1].isnumeric()
                or not any([data.isnumeric() for data in line_data[2].split(".")])
                or not any([data.isnumeric() for data in line_data[4].split(".")])
                or not line_data[6].isnumeric()
            ):
                return False
    ...
    def check_rmc_integrity(line_data: list[str]) -> bool:
        ...
        try:
            ...
            if (
                line_data[2].isnumeric()
                or line_data[4].isnumeric()
                or line_data[6].isnumeric()
            ):
                return False
            ...
            if (
                not line_data[1].isnumeric()
                or not any([data.isnumeric() for data in line_data[3].split(".")])
                or not any([data.isnumeric() for data in line_data[7].split(".")])
                or not line_data[5].isnumeric()
            ):
                return False
